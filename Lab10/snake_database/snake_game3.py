...
last_score_here, last_level_here = get_last_score(user_id)

# Start with previous progress if desired
score = last_score_here
level = last_level_here
speed = 200 - (level - 1) * 20
...
def game_over():
    global done
    save_score(user_id, score, level)  # Save progress
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_p:  # Press 'P' to pause
                ...
                print(f"Game paused. Progress saved for user: {username}")
                while paused:
                    for pause_event in pygame.event.get():
                        if pause_event.type == pygame.KEYDOWN and pause_event.key == pygame.K_r:
                            paused = False
                            print("Game resumed.")
